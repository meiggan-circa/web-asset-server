    ...
    if settings.TIME_TOLERANCE is not None:
        ...
        if not abs(current_time - timestamp) < settings.TIME_TOLERANCE:
            raise TokenException(
                "Auth token timestamp out of range: %s vs %s"
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if (
                ...
                or settings.REQUIRE_KEY_FOR_GET
            ):
...
def include_timestamp(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        (result if isinstance(result, Response) else response).set_header(
...
def hdfs_path_exists(hdfs_path):
    response = requests.get(f"{settings.HDFS_API}/exists?path={hdfs_path}")
    return response.ok


def create_hdfs_dir(hdfs_dir):
    requests.post(f"{settings.HDFS_API}/mkdir/", data={"path": hdfs_dir})
...
def upload_to_hdfs(hdfs_path, file):
    requests.post(f"{settings.HDFS_API}/upload/", data={"path": hdfs_path}, files=file)
...
def fileget():
    ...
    if download_name:
        ...
        response.set_header(
            "Content-Disposition", f"inline; filename*=utf-8''{download_name}"
        )
    return data
...
def filedelete():
    ...
    requests.post(f"{settings.HDFS_API}/delete/", data={"path": orig_path})
...
def web_asset_store():
    ...
    return template(
        "web_asset_store.xml",
        host="%s:%d" % (settings.SERVER_NAME, settings.SERVER_PORT),
    )
